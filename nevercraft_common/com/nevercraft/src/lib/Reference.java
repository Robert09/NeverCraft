package NeverCraft.nevercraft_common.com.nevercraft.src.lib;

public class Reference {
    public static final String MOD_ID = "nevercraft";
    public static final String MOD_NAME = "NeverCraft";
    public static final String MOD_VERSION = "pre 0.1";
    
    //Proxys
    public static final String SERVER_PROXY_CLASS= "com.nevercraft.src.CommonProxy";
    public static final String CLIENT_PROXY_CLASS= "com.nevercraft.src.ClientProxy";
}
